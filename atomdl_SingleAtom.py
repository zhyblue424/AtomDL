...
from tqdm import tqdm
from utils import atom_localization
def parse_option():
    ...
    parser.add_argument('save_path', default="./AtomDL/", type=str, help='Path to the folder where results will be saved')
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(files_root_W))):
        ...
        coords_xy = np.array(centers)[:, [1, 0]]
        ...
        """Identify single-atom sites based on different quantiles"""
        quantiles = [99.5, 99.0, 98.0, 95.0]
        for quantile in quantiles:
            os.makedirs(os.path.join(save_path_single, f"{100-quantile}highlight"), exist_ok=True)
            threshold = np.percentile(avg_intensities[avg_intensities > 0], quantile)
            mask = avg_intensities[:, 0] > threshold
            filtered_blobs = blobs_with_intensity[mask]
            filtered_coords = filtered_blobs[:, [1, 0]]
            filtered_dist_matrix = dist_matrix[mask, :]
            highlight_groups = []
            image = Image.open(in_path).convert("RGB") 
            image_np = np.array(image)

            atom_localization(X, Y, filtered_blobs, image_np, filtered_dist_matrix, coords_xy, highlight_groups, save_path_single, quantile = quantile, single_atom = True)
            
            

            """normal atom"""
            threshold = np.mean(avg_intensities)
            threshold = np.mean(avg_intensities[avg_intensities > 0])
            mask = (avg_intensities[:, 0] > 0) & (avg_intensities[:, 0] < threshold)
            low_blobs = blobs_with_intensity[mask]
            random_indices = np.random.choice(len(low_blobs), size=100, replace=False)
            number = np.sort(random_indices)
            filtered_blobs = low_blobs[number]
            filtered_coords = filtered_blobs[:, [1, 0]]
            filtered_dist_matrix = dist_matrix[mask, :][number, :]

            highlight_groups = []
            image = Image.open(in_path).convert("RGB") 
            image_np = np.array(image)

            atom_localization(X, Y, filtered_blobs, image_np, filtered_dist_matrix, coords_xy, highlight_groups, save_path_single, single_atom = False)
